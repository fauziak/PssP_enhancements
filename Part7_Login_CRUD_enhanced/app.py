...
def get_gui_patients():
    if 'loggedin' in session and (session['account_type'] == 'admin' or session['account_type'] == 'careprovider'):
...
def updateAccount(): # note this function needs to match name in html form action
    form_account_id = request.form.get('id')
    user = Users.query.filter_by(id=form_account_id).first()
